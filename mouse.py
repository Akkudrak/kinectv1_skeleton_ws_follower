...
JOINT_RIGHT_HAND = 7
JOINT_RIGHT_ELBOW = 5
JOINT_RIGHT_SHOULDER = 3  # Hombro derecho
...
# Suavizado de coordenadas en 2D
smooth_joints = {}
smooth_alpha = 0.2

# Umbral de alejamiento 3D para click
DISTANCE_THRESHOLD = 300  # mm (40 cm)
...
#print("Ella: Kinect v1 + OpenNI2 + NiTE2 con click/drag por alejamiento de la mano derecha.")
        ...
        for user in user_frame.users:
            if user.is_new():
                ...
                if right_hand.positionConfidence > 0.5 and right_shoulder.positionConfidence > 0.5:
                    ...
                    # Distancia 3D entre mano y hombro (en mm)
                    dx = right_hand.position.x - right_shoulder.position.x
                    dy = right_hand.position.y - right_shoulder.position.y
                    dz = right_hand.position.z - right_shoulder.position.z
                    distancia_3d = math.sqrt(dx*dx + dy*dy + dz*dz)

                    # Control de click/drag
                    if distancia_3d > DISTANCE_THRESHOLD and not clicking:
                        #print(f"Click presionado (distancia: {distancia_3d:.0f} mm)")
                        click_mouse(True)
                    elif distancia_3d <= DISTANCE_THRESHOLD and clicking:
                        #print(f"Click liberado (distancia: {distancia_3d:.0f} mm)")
                        click_mouse(False)

                    cv2.putText(frame_bgr, f"Dist: {distancia_3d:.0f} mm", (20, 40),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

        cv2.imshow("Kinect v1 - Click/Drag por alejamiento", frame_bgr)
